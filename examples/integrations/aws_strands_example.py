...
class ProfessionalCoachingTools:
    ...
    @tool
    def set_career_goal(
        self, goal: str, target_date: str, priority: str = "medium"
    ) -> str:
    ...
    def track_learning_progress(
        self, activity: str, status: str, notes: str = ""
    ) -> str:
    ...
    def recommend_resources(self, topic: str, resource_type: str = "any") -> str:
        ...
        try:
            ...
            recommendations = {
                ...
                "leadership": {
                    "course": [
                        "Leadership Fundamentals (LinkedIn Learning)",
                ...
                if key in topic_lower or topic_lower in key:
                    if resource_type == "any":
                        for res_type, items in resources.items():
                            resource_recommendations.extend(
                                [f"{res_type.title()}: {item}" for item in items]
                            )
                    elif resource_type in resources:
                        resource_recommendations.extend(
                            [
                                f"{resource_type.title()}: {item}"
...
def initialize_memory_system():
    ...
    try:
        ...
        database_url = os.getenv(
            "DATABASE_URL", "sqlite:///professional_coach_memory.db"
        )
        ...
        if not openai_api_key:
            logger.warning(
                "OpenAI API key not found. Memory intelligence features may be limited."
            )
...
def create_coaching_agent(memori_instance):
    ...
    agent = Agent(model=model, tools=all_tools, name="Professional Development Coach")
...
async def main():
        ...
        while True:
            try:
                ...
                if user_input.lower() in ["quit", "exit", "bye", "goodbye"]:
                    ...
                    print(
                        "Your progress and goals are saved for our next conversation."
                    )
                    print(
                        "Keep up the excellent work on your professional development! 🚀"
                    )
                    break
                ...
                try:
                    ...
                    memori.record_conversation(
                        user_input=user_input, ai_output=str(result.message)
                    )
    ...
    print("   • Memory database: professional_coach_memory.db")
    print("   • Namespace: professional_coach")
