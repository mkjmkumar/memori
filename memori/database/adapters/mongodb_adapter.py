...
class MongoDBAdapter:
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            lt_collection.create_index([("searchable_content", "text"), ("summary", "text")], background=True)
    ...
    def _convert_memory_to_document(self, memory_data: Dict[str, Any]) -> Dict[str, Any]:
        """Convert memory data to MongoDB document format"""
        document = memory_data.copy()
        
        # Ensure datetime fields are datetime objects
        datetime_fields = ['created_at', 'expires_at', 'last_accessed', 'extraction_timestamp']
        ...
        json_fields = ['processed_data', 'entities_json', 'keywords_json', 'supersedes_json', 'related_memories_json', 'metadata']
    ...
    def store_chat_interaction(
        ...
        try:
            ...
            return self.update_chat_interaction(chat_id, user_input, ai_output, model, tokens_used, metadata)
        except Exception as e:
    ...
    def search_memories(
        ...
        try:
            ...
            if not memory_types or 'short_term' in memory_types:
                collections_to_search.append((self.SHORT_TERM_MEMORY_COLLECTION, 'short_term'))
            if not memory_types or 'long_term' in memory_types:
                collections_to_search.append((self.LONG_TERM_MEMORY_COLLECTION, 'long_term'))
            ...
            for collection_name, memory_type in collections_to_search:
                ...
                ).sort([('score', {'$meta': 'textScore'}), ('importance_score', -1)]).limit(limit)
    ...
    def batch_store_memories(
        ...
        try:
            ...
            logger.info(f"Batch stored {len(result.inserted_ids)} {memory_type} memories")
            return memory_ids
    ...
    def cleanup_expired_memories(self, namespace: str = "default") -> int:
        ...
        try:
            ...
            if count > 0:
                logger.info(f"Cleaned up {count} expired memories from namespace: {namespace}")
    ...
    def get_memory_stats(self, namespace: str = "default") -> Dict[str, Any]:
        ...
        try:
            ...
            stats['short_term_count'] = self.database[self.SHORT_TERM_MEMORY_COLLECTION].count_documents({
                'namespace': namespace
            })
            
            stats['long_term_count'] = self.database[self.LONG_TERM_MEMORY_COLLECTION].count_documents({
                'namespace': namespace
            })
            
            stats['chat_history_count'] = self.database[self.CHAT_HISTORY_COLLECTION].count_documents({
                'namespace': namespace
            })
