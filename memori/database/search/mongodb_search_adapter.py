...
class MongoDBSearchAdapter(BaseSearchAdapter):
    ...
    def execute_hybrid_search(
        ...
        try:
            ...
            return self.execute_fulltext_search(query, namespace, category_filter, limit)
    ...
    def _combine_search_results(
        ...
        for result in vector_results:
            ...
            if memory_id:
                ...
                if memory_id in seen_memories:
                    ...
                    vector_combined = (vector_score * vector_weight) + (importance_score * 0.1)
                    ...
                    combined_score = (vector_score * vector_weight) + (importance_score * 0.1)
        ...
        logger.debug(f"Hybrid search combined {len(text_results)} text + {len(vector_results)} vector results")
            ...
            for collection, collection_name in collections:
                try:
                    ...
                    collection.create_index([
                        ...
                        ('summary', 'text')
                    ], name=f'{collection_name}_text_search', background=True)
                    
                    indexes_created.append(f'{collection_name}_text_search')
                    logger.info(f"Created text index for {collection_name}")
                    
                except Exception as e:
                    logger.warning(f"Failed to create text index for {collection_name}: {e}")
            
            # Note about vector indexes
            if self.mongodb_connector.supports_vector_search():
                logger.info("Vector search is supported. Create vector indexes via MongoDB Atlas UI or Admin API.")
    ...
    def _check_vector_search_available(self) -> bool:
        ...
        try:
            ...
            self._vector_search_available = self.mongodb_connector.supports_vector_search()
            ...
            if self._vector_search_available:
                try:
                    ...
                    if "vector search" in str(e).lower() or "index" in str(e).lower():
                        logger.warning("Vector search is supported but not configured (missing indexes)")
        ...
        try:
            ...
            for collection in collections:
                try:
                    ...
                    logger.debug(f"Collection {collection.name} stats: {stats.get('count', 0)} documents")
                    
                    # List indexes to ensure they exist
                    indexes = list(collection.list_indexes())
                    logger.debug(f"Collection {collection.name} has {len(indexes)} indexes")
