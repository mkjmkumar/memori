...
class MongoDBConnector(BaseDatabaseConnector):
    ...
    def __init__(self, connection_config):
        ...
        if not PYMONGO_AVAILABLE:
            raise DatabaseError("pymongo is required for MongoDB support. Install with: pip install pymongo")
        ...
        try:
            ...
            if isinstance(query, str) and query.strip().startswith('{'):
                ...
                if operation_type == 'find':
                    ...
                    raise DatabaseError(f"Unsupported MongoDB operation: {operation_type}")
            else:
        ...
        try:
            ...
            try:
                ...
                if "Transaction numbers" in str(e):
                    ...
                    logger.warning("Transactions not supported, executing operations individually")
                    for query, params in queries:
                        if 'insert' in query.lower():
                            self.execute_insert(query, params)
                        elif 'update' in query.lower():
    ...
    def initialize_schema(self, schema_sql: Optional[str] = None):
        ...
        try:
            from ..schema_generators.mongodb_schema_generator import MongoDBSchemaGenerator
    ...
    def _is_atlas_connection(self) -> bool:
        ...
        return (
            "mongodb.net" in self.connection_string.lower() or
            "atlas" in self.connection_string.lower() or
            "cluster" in self.connection_string.lower()
        )
    ...
    def create_vector_index(
        self, collection_name: str, vector_field: str, dimensions: int, 
        similarity: str = "cosine", index_name: Optional[str] = None
    ) -> str:
        """Create MongoDB Atlas Vector Search index"""
        try:
            if not self.supports_vector_search():
                raise DatabaseError("Vector search is not supported in this MongoDB deployment")
            ...
            logger.warning("Vector search indexes should be created via MongoDB Atlas UI or Admin API")
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            info["connection_string"] = self.connection_string.replace(
                f"{self.username}:{self.password}@", "***:***@"
            ) if self.username and self.password else self.connection_string
