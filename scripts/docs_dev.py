...
class MkDocsDevHelper:
    ...
    def run_command(self, cmd: List[str], capture_output: bool = False, check: bool = True) -> subprocess.CompletedProcess:
    ...
    def check_dependencies(self) -> bool:
        ...
        try:
            result = self.run_command(["mkdocs", "build", "--strict", "--clean"], 
                                    capture_output=True, check=False)
            if result.returncode == 0:
                self.print_status("All MkDocs dependencies are working correctly!", "SUCCESS")
                return True
        except FileNotFoundError:
            pass
        
        self.print_status("Some dependencies may be missing. Checking individually...", "WARNING")
        return False
    ...
    def install_dependencies(self) -> bool:
        ...
        try:
            ...
            install_cmd = [sys.executable, "-m", "pip", "install"] + self.required_packages
    ...
    def build_docs(self, clean: bool = True, strict: bool = True) -> bool:
        ...
        try:
            ...
            self.print_status(f"Documentation built successfully in {self.site_dir}", "SUCCESS")
    ...
    def serve_docs(self, host: str = "127.0.0.1", port: int = 8000, open_browser: bool = True) -> None:
    ...
    def show_stats(self) -> None:
        ...
        print(f"  • Average words per file: {total_words // len(md_files) if md_files else 0:,}")
...
Examples:
  ...
  python docs_dev.py --clean            # Clean build artifacts
    ...
    parser.add_argument("--host", default="127.0.0.1", 
                       help="Host for dev server (default: 127.0.0.1)")
    ...
    try:
        ...
        if not helper.check_dependencies():
            if args.install_only or input("\nInstall missing dependencies? [Y/n]: ").lower() != 'n':
                if not helper.install_dependencies():
                    sys.exit(1)
                helper.print_status("Dependencies installed! Please run the script again.", "SUCCESS")
        ...
        if not helper.validate_mkdocs_config():
            helper.print_status("Please fix configuration issues before continuing", "ERROR")
            sys.exit(1)
        ...
        if args.serve or (not any([args.install_only, args.build, args.check, args.clean, args.stats])):
